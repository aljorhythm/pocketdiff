#!/usr/bin/env node
// GENERATED by 'npm run bundle' from bin/cli.js + src/* — do not edit by hand.
"use strict";
...
var require_gitdiff_parser = __commonJS({
  "node_modules/gitdiff-parser/index.js"(exports2, module2) {
    (function(root) {
      var STAT_START = 2;
      ...
      if (typeof exports2 === "object" && typeof module2 === "object") {
        exports2 = module2.exports = parser2;
      } else if (typeof define === "function" && define.amd) {
        define("gitDiffParser", [], parser2);
      } else {
        root.gitDiffParser = parser2;
      }
    })(exports2);
  }
});
...
`;
    module2.exports = { langFor, highlightLine, overlayChanged, HL_CSS };
  }
});
...
var require_render = __commonJS({
  "src/render.js"(exports2, module2) {
    ...
    var { classify, group, layerOf, domainKeyOf, LAYER_ORDER } = require_group();
    var { langFor, highlightLine, overlayChanged, HL_CSS } = require_highlight();
    ...
    function segsHtml(segs) {
      return segs.map((s) => s.changed ? `<span class="wq">${esc(s.text)}</span>` : esc(s.text)).join("");
    }
    ...
    function renderHunk(hunk, lang, hi) {
      ...
      for (let k = 0; k < ch.length; ) {
        if (ch[k].type === "delete" || ch[k].type === "insert") {
          ...
          dels.forEach(
            (d, p) => rows += row("del", "-", d.lineNumber, codeCell(d.content, p < pairs ? diffs[p].aSegs : null, lang, hi))
          );
          ins.forEach(
            (d, p) => rows += row("ins", "+", d.lineNumber, codeCell(d.content, p < pairs ? diffs[p].bSegs : null, lang, hi))
          );
        } else {
          const c = ch[k++];
    ...
    function renderFile(file, hi) {
      ...
      if (file.binary) {
        ...
        const skip = `<tr class="skip" title="Only changed sections are in a diff \u2014 the full file isn't included"><td class="ln" aria-hidden="true">\u22EF</td><td class="code">unchanged lines not shown</td></tr>`;
...
<details class="file" id="${file.id}"${open} data-path="${esc(file.path)}"${noiseAttr}${groupAttrs(file)}>
  ...
  ${body}
    ...
    function fileListEntry(file) {
      ...
      const md2 = markdown ? '<span class="badge md">md</span>' : "";
    ...
    function renderMarkdownBar(files) {
      ...
      const chips = mds.map(
        (f) => `<a class="md-chip" href="#prev-${f.id}" data-path="${esc(f.path)}">${esc(basename(f.path))}</a>`
      ).join("");
...
<details class="group" open data-dir="${esc(label)}">
  ...
  ${g.files.map((f) => renderFile(f, hi)).join("")}
    ...
    function render2(rawFiles, opts = {}) {
      ...
      const gbtn = (m, label) => `<button type="button" data-group="${m}"${gmode === m ? ' class="active" aria-pressed="true"' : ' aria-pressed="false"'}>${label}</button>`;
      return `<!DOCTYPE html>
...
<footer>Generated by pocketdiff \xB7 ${generated}</footer>
...
`;
    module2.exports = { render: render2, esc };
  }
});
...
var require_input = __commonJS({
  "src/input.js"(exports2, module2) {
    "use strict";
    var fs2 = require("fs");
    var strip = (s) => s.replace(/\/$/, "").replace(/\.(diff|patch)$/, "");
    ...
    function bitbucketDiffUrl(url) {
      ...
      const api = (ws, repo, rest) => `https://api.bitbucket.org/2.0/repositories/${ws}/${repo}/${rest}`;
      let m;
    ...
    function classifyInput2(token) {
      if (/^https?:\/\//i.test(token)) return { kind: "url", value: token };
      try {
        if (fs2.existsSync(token) && fs2.statSync(token).isFile())
          return { kind: "file", value: token };
      } catch {
      }
      return { kind: "git", value: token };
    }
    module2.exports = { toDiffUrl: toDiffUrl2, classifyInput: classifyInput2 };
  }
});
...
var parser = require_gitdiff_parser();
var { render } = require_render();
var { toDiffUrl, classifyInput } = require_input();
var HELP = `pocketdiff \u2014 turn a diff into a self-contained HTML file
...
async function fetchDiff(input) {
  ...
  const headers = {
    ...
    Accept: isGitHubApi ? "application/vnd.github.v3.diff" : "application/vnd.github.v3.diff, text/plain, */*"
  };
  ...
  try {
    ...
    throw new Error(
      `could not reach ${url}: ${e.message}. Check your network/proxy, or generate the diff locally (\`git diff <range>\`) and pipe it in instead.`
    );
  }
  if (!res.ok) {
...
function recoveryHint(status, provider, tokenEnv, tokenPresent) {
  ...
  if (status === 401 || status === 403) {
    ...
    return tokenPresent ? ` \u2014 ${provider} rejected the token. It may be expired or lack access; use ${scopes[provider]} that can read this repo.` : ` \u2014 looks private/forbidden. Set ${tokenEnv} to ${scopes[provider]} and retry.`;
  }
  if (status === 404) {
    if (!provider) return " \u2014 not found. Check the URL, or pass a raw .diff URL / pipe a diff instead.";
    return tokenPresent ? " \u2014 not found. Double-check the URL; the token may also lack access to a private repo." : ` \u2014 not found, or the repo is private. Verify the URL; if private, set ${tokenEnv} and retry.`;
  }
