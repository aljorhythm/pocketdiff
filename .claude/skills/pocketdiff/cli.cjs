    ...
    function renderHunk(hunk) {
      ...
      for (let k = 0; k < ch.length; ) {
        if (ch[k].type === "delete" || ch[k].type === "insert") {
          ...
          dels.forEach(
            (d, p) => rows += row("del", "-", d.lineNumber, p < pairs ? diffs[p].oa : esc(d.content))
          );
          ins.forEach(
            (d, p) => rows += row("ins", "+", d.lineNumber, p < pairs ? diffs[p].ob : esc(d.content))
          );
        } else {
          const c = ch[k++];
    ...
    function renderFile(file) {
      ...
      if (file.binary) {
        ...
        if (isMarkdown(file.path)) {
          ...
          const id = file.id;
          body = `<div class="vtabs"><a class="vtab" href="#${id}">Diff</a><a class="vtab" href="#prev-${id}">Preview</a></div><div id="prev-${id}" class="preview markdown">${preview}<p class="preview-note">Preview of changed sections (new version).</p></div>` + table;
        } else {
          body = table;
        }
      }
